...
class RenderPositions(RunScene):
	...
	def __init__(self, scenefilename, filename,
		...
		camera1video,
		videowriter=None):
		super(RenderPositions, self).__init__(scenefilename, videowriter)
	...
	def loadData(self):
		...
		handler 	= open(self._filename, 'rb')
		...
		self._video0.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, self._frames[0])
		self._video1.set(cv2.cv.CV_CAP_PROP_POS_FRAMES, self._frames[0])
